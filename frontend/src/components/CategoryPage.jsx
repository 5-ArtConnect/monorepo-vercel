...
export default function CategoryPage() {
  ...
  // Fetch artworks by category from API
  useEffect(() => {
    const fetchCategoryArtworks = async () => {
      try {
        setLoading(true);
        // Capitalize category name for API
        const capitalizedCategory = categoryName.charAt(0).toUpperCase() + categoryName.slice(1);
        const artworksList = await getArtworksByCategory(capitalizedCategory);
        console.log('Category artworks:', artworksList); // Debug
  ...
  return (
    <>
      ...
      <section className="bg-[#FAF7F0] py-16 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <h1 className="text-5xl font-bold mb-6 text-gray-800">
            {categoryName.charAt(0).toUpperCase() + categoryName.slice(1)} Art Collection
          </h1>
          <p className="text-xl text-gray-600 max-w-4xl mx-auto">
            Explore amazing {categoryName} artworks from talented artists around the world.
          </p>
        </div>
      </section>
              ...
              >
                <div className="h-64 overflow-hidden">
                  <img 
                    ...
                    onError={(e) => {
                      e.target.src = imp1; // Fallback image
                    }}
                  />
                </div>
                <div className="p-6">
                  <h3 className="text-xl font-bold text-gray-800 mb-2">{artwork.title}</h3>
                  <p className="text-[#463b33] font-semibold mb-1">
      ...
      {/* Back Button Section */}
      <section className="w-full bg-[#FAF7F0] py-8 px-4">
        <div className="max-w-6xl mx-auto text-center">
          <button 
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                  <img 
                    src={selectedArt.image_url || imp1} 
                ...
                <div>
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-gray-700">{selectedArt.artist_name || 'Unknown Artist'}</p>
                    </div>
                    
                    <div>
                      <h3 className="text-lg font-semibold text-[#463b33]">Category</h3>
                      <p className="text-gray-700">{selectedArt.category}</p>
                    </div>
                    
                    <div>
                      <h3 className="text-lg font-semibold text-[#463b33]">Year</h3>
                      <p className="text-gray-700">{new Date(selectedArt.created_at).getFullYear()}</p>
                    </div>
